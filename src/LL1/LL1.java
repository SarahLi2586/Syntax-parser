...
public class LL1 {
    ...
    private void control(){
        while (flag) {
            if (isTerminal(stackpeek())) { //栈顶为terminal
                ...
                } else if (stackpeek().equals(currChar())) { //栈和表达式字符相等，栈pop, 表达式向后遍历
                    currindex++;
                    stack.pop();
            ...
            } else { //nonterminal的话就得到对应的表达式存进栈里
                if (currChar().equals("$")) { //表达式结束但是栈还没结束看对应的表达式是否为ε
                    if (getRule().equals("ε")) {
                        stack.pop();
                        System.out.printf("%-20s %6s %-1s->%-12s\n", stack.toString(), input.substring(currindex, input.length()), top, getRule());
                    }
